...
BEGIN
  ...
  etapa_events AS (
    SELECT
      a.usuario_id AS uid,
      a.lead_id,
      substring(a.descricao FROM 'para\s+"([^"]+)"') AS etapa_para
  ...
  etapa_stats AS (
    SELECT
      uid,
      -- Lógica progressiva (funil): contato realizado considera quem chegou a contato_realizado OU reuniao_agendada
