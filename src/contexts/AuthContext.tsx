...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserData = useCallback(async (user: User, attempt = 0): Promise<void> => {
  ...
  useEffect(() => {
    const {
      ...
      if (session?.user) {
        // Ignora eventos que não trazem novidade (token refresh ou re-emissão da sessão atual)
        if (
  ...
  const hasPageAccess = useCallback(
    ...
    [state.isAdmin, state.allowedPages],
  );
