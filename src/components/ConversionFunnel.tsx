...
const ConversionFunnel = ({
  ...
  return <div className="relative grid grid-cols-[1fr_4fr_1fr] gap-6">
      ...
      <div className="space-y-4">
        ...
        return <div key={index} className="rounded-lg bg-gradient-to-br from-card to-muted/5 border border-border/50 p-5 transition-all duration-300 hover:shadow-lg hover:border-primary/30">
              ...
              <div className="mb-3 h-2 overflow-hidden rounded-full bg-muted/30">
                <div className={`h-full transition-all duration-500 ${stage.barColor}`} style={{
              width: `${barWidth}%`
            }} />
              </div>
              ...
              <div className="flex items-center justify-between">
                {stage.conversionRate > 0 && <span className="font-body text-xs text-muted-foreground">
                ...
                <span className={`font-body text-xs text-muted-foreground ${stage.conversionRate === 0 ? 'ml-auto' : ''}`}>
                  {stage.costLabel}: <span className="font-semibold text-foreground">R$ {stage.costValue.toLocaleString("pt-BR", {
      ...
      <div className="flex flex-col justify-around">
        {intermediateRates.map((rate, index) => <div key={index} className="text-center">
