  ...
  return <div className="rounded-lg border border-border/50 bg-gradient-to-br from-card to-muted/5 p-6 transition-all duration-300 hover:shadow-lg">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="outline" onClick={() => setMonthPreset(2)} className="h-7 text-xs">
            2 Meses Atrás
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-9">
        ...
        <div>
          ...
          <Select value={filters.canal} onValueChange={v => onFilterChange("canal", v)}>
            ...
            <SelectContent>
              ...
              {uniqueValues.canais.map(canal => <SelectItem key={canal} value={canal}>{canal}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={filters.tier} onValueChange={v => onFilterChange("tier", v)}>
            ...
            <SelectContent>
              <SelectItem value="all">Todos</SelectItem>
              {uniqueValues.tiers.map(tier => <SelectItem key={tier} value={tier}>{tier}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={filters.urgency} onValueChange={v => onFilterChange("urgency", v)}>
            ...
            <SelectContent>
              ...
              {uniqueValues.urgencias.map(urgencia => <SelectItem key={urgencia} value={urgencia}>{urgencia}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={filters.cargo} onValueChange={v => onFilterChange("cargo", v)}>
            ...
            <SelectContent>
              <SelectItem value="all">Todos</SelectItem>
              {uniqueValues.cargos.map(cargo => <SelectItem key={cargo} value={cargo}>{cargo}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Per. Compra      </label>
          <Select value={filters.periodo} onValueChange={v => onFilterChange("periodo", v)}>
            ...
            <SelectContent>
              <SelectItem value="all">Todos</SelectItem>
              {uniqueValues.periodos.map(periodo => <SelectItem key={periodo} value={periodo}>{periodo}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
