...
export const OportunidadeTimeline = ({
  oportunidadeId,
  hideNotaComposer,
  tarefaDialogOpen,
  ...
  const { data: atividadesAll = [], addNota, addTarefa, toggleTarefa, remove } =
    useOportunidadeAtividades(oportunidadeId);
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <Input
              placeholder="Título da tarefa (ex: Enviar contrato revisado)"
              value={tarefaTitulo}
...
const TimelineItem = ({
  ...
  return (
    <li className="ml-4 group">
      <span className={`absolute -left-[11px] flex items-center justify-center h-5 w-5 rounded-full border ${meta.color}`}>
      ...
      <div className="bg-muted/10 border border-border/40 rounded-lg p-3 hover:bg-muted/20 transition-colors">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            {isTarefa ? (
              <button onClick={() => onToggle(!a.concluida)} className="mt-1.5 flex items-start gap-2 text-left w-full">
                {a.concluida ? (
              ...
              a.descricao && (
                <p className="text-sm text-foreground mt-1 whitespace-pre-wrap break-words">{a.descricao}</p>
              )
            )}
